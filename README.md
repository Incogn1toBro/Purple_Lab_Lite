...
PLEASE NOTE: This deployment contains a highly vulnerable web server. It is within the user's best interest to utilise a host only network when working with this instance.

1. Install Docker (ideally inside a VM)
...
6. Build the Docker containers ```sudo docker compose up -d```
7. Confirm the containers are running ```sudo docker ps```
